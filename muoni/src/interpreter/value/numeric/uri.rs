...
impl Numeric for URI {
    ...
    fn to_rfm(self: Box<Self>) -> RFM {
        if self.1.is_empty() {
            DMatrix::from_element(1,1,self.0 as RF)
        } else {
            panic!("cannot discard units")
        }
    }
    ...
    fn to_cb(self: Box<Self>) -> CB {
        if self.1.is_empty() {
            Complex::from(BigInt::from(self.0))
        } else {
            panic!("cannot discard units")
        }
    }
    ...
    fn to_cfm(self: Box<Self>) -> CFM {
        if self.1.is_empty() {
            DMatrix::from_element(1,1,Complex::from(self.0 as RF))
        } else {
            panic!("cannot discard units")
        }
    }
