...
impl Numeric for UCFM {
    ...
    fn to_rim(self: Box<Self>) -> RIM {
        if self.1.is_empty() {
            self.0.map(|x| if x.im == 0. {x.re as RI} else {panic!("cannot discard imaginary part")})
        } else {
            panic!("cannot discard units")
        }
    }
    ...
    fn to_urb(self: Box<Self>) -> URB {
        if self.0.data.len() == 1 {
            unsafe {
                let n = *self.0.data.get_unchecked(0);
                if n.im == 0. {
                    UT(BigInt::from(n.re as RI),self.1)
                } else {
                    panic!("cannot discard imaginary part")
                }
            }
    ...
    fn to_urim(self: Box<Self>) -> URIM {
        UT(self.0.map(|x| if x.im == 0. {x.re as RI} else {panic!("cannot discard imaginary part")}),self.1)
    }
    ...
    fn to_cim(self: Box<Self>) -> CIM {
        if self.1.is_empty() {
            self.0.map(|x| Complex::new(x.re as RI, x.im as RI))
        } else {
            panic!("cannot discard units")
        }
    }
    ...
    fn to_uci(self: Box<Self>) -> UCI {
        if self.0.data.len() == 1 {
            unsafe {
                let n = *self.0.data.get_unchecked(0);
                UT(Complex::new(n.re as RI, n.im as RI),self.1)
            }
        } else if self.0.data.len() == 0 {
            panic!("cannot convert empty matrix")
        } else {
    ...
    fn to_ucb(self: Box<Self>) -> UCB {
        if self.0.data.len() == 1 {
            unsafe {
                let n = *self.0.data.get_unchecked(0);
                UT(Complex::new(BigInt::from(n.re as RI), BigInt::from(n.im as RI)),self.1)
            }
        } else if self.0.data.len() == 0 {
            panic!("cannot convert empty matrix")
        } else {
    ...
    fn to_ucim(self: Box<Self>) -> UCIM {
        UT(self.0.map(|x| Complex::new(x.re as RI, x.im as RI)),self.1)
    }
