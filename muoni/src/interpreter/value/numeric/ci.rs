...
impl Numeric for CI {
    ...
    fn to_rfm(self: Box<Self>) -> RFM {
        if self.im == 0 {
            DMatrix::from_element(1,1,self.re as RF)
        } else {
            panic!("cannot discard imaginary part")
        }
    }
    ...
    fn to_urb(self: Box<Self>) -> URB {
        if self.im == 0 {
            UT(BigInt::from(self.re),D::empty())
        } else {
            panic!("cannot discard imaginary part")
        }
    }
    ...
    fn to_urfm(self: Box<Self>) -> URFM {
        if self.im == 0 {
            UT(DMatrix::from_element(1,1,self.re as RF),D::empty())
        } else {
            panic!("cannot discard imaginary part")
        }
    }
    ...
    fn to_cf(self: Box<Self>) -> CF {
        Complex::new(self.re as RF, self.im as RF)
    }
    fn to_cb(self: Box<Self>) -> CB {
        Complex::new(BigInt::from(self.re),BigInt::from(self.im))
    }
    fn to_cim(self: Box<Self>) -> CIM {
        DMatrix::from_element(1,1,*self)
    }
    fn to_cfm(self: Box<Self>) -> CFM {
        DMatrix::from_element(1,1,Complex::new(self.re as RF, self.im as RF))
    }
    fn to_uci(self: Box<Self>) -> UCI {
        UT(*self,D::empty())
    }
    fn to_ucf(self: Box<Self>) -> UCF {
        UT(Complex::new(self.re as RF, self.im as RF),D::empty())
    }
    fn to_ucb(self: Box<Self>) -> UCB {
        UT(Complex::new(BigInt::from(self.re),BigInt::from(self.im)),D::empty())
    }
    fn to_ucim(self: Box<Self>) -> UCIM {
        UT(DMatrix::from_element(1,1,*self),D::empty())
    }
    fn to_ucfm(self: Box<Self>) -> UCFM {
        UT(DMatrix::from_element(1,1,Complex::new(self.re as RF, self.im as RF)),D::empty())
    }
