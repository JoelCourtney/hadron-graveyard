...
impl Numeric for UCI {
    ...
    fn to_rfm(self: Box<Self>) -> RFM {
        if self.1.is_empty() {
            if self.0.im == 0 {
                DMatrix::from_element(1,1,self.0.re as RF)
            } else {
                panic!("cannot discard imaginary part")
            }
        } else {
    ...
    fn to_urb(self: Box<Self>) -> URB {
        if self.0.im == 0 {
            UT(BigInt::from(self.0.re),self.1)
        } else {
            panic!("cannot discard imaginary part")
        }
    }
    ...
    fn to_urfm(self: Box<Self>) -> URFM {
        if self.0.im == 0 {
            UT(DMatrix::from_element(1,1,self.0.re as RF),self.1)
        } else {
            panic!("cannot discard imaginary part")
        }
    }
    ...
    fn to_cf(self: Box<Self>) -> CF {
        if self.1.is_empty() {
            Complex::new(self.0.re as RF, self.0.im as RF)
        } else {
            panic!("cannot discard units")
        }
    }
    fn to_cb(self: Box<Self>) -> CB {
        if self.1.is_empty() {
            Complex::new(BigInt::from(self.0.re),BigInt::from(self.0.im))
        } else {
            panic!("cannot discard units")
        }
    }
    ...
    fn to_cfm(self: Box<Self>) -> CFM {
        if self.1.is_empty() {
            DMatrix::from_element(1,1,Complex::new(self.0.re as RF, self.0.im as RF))
        } else {
            panic!("cannot discard units")
        }
    }
    ...
    fn to_ucf(self: Box<Self>) -> UCF {
        UT(Complex::new(self.0.re as RF, self.0.im as RF),self.1)
    }
    fn to_ucb(self: Box<Self>) -> UCB {
        UT(Complex::new(BigInt::from(self.0.re),BigInt::from(self.0.im)),self.1)
    }
    fn to_ucim(self: Box<Self>) -> UCIM {
        UT(DMatrix::from_element(1,1,self.0),self.1)
    }
    fn to_ucfm(self: Box<Self>) -> UCFM {
        UT(DMatrix::from_element(1,1,Complex::new(self.0.re as RF, self.0.im as RF)),self.1)
    }
