...
impl Numeric for URIM {
    ...
    fn to_rf(self: Box<Self>) -> RF {
        if self.0.data.len() == 1 {
            if self.1.is_empty() {
                unsafe {
                    *self.0.data.get_unchecked(0) as RF
                }
            } else {
                panic!("cannot discard units")
            }
    ...
    fn to_rb(self: Box<Self>) -> RB {
        if self.0.data.len() == 1 {
            if self.1.is_empty() {
                unsafe {
                    BigInt::from(*self.0.data.get_unchecked(0))
                }
            } else {
                panic!("cannot discard units")
            }
    ...
    fn to_urf(self: Box<Self>) -> URF {
        if self.0.data.len() == 1 {
            unsafe {
                UT(*self.0.data.get_unchecked(0) as RF,self.1)
            }
        } else if self.0.data.len() == 0 {
            panic!("cannot convert empty matrix")
        } else {
    ...
    fn to_urb(self: Box<Self>) -> URB {
        if self.0.data.len() == 1 {
            unsafe {
                UT(BigInt::from(*self.0.data.get_unchecked(0)),self.1)
            }
        } else if self.0.data.len() == 0 {
            panic!("cannot convert empty matrix")
        } else {
    ...
    fn to_urfm(self: Box<Self>) -> URFM {
        UT(self.0.map(|x| x as RF),self.1)
    }
    ...
    fn to_cf(self: Box<Self>) -> CF {
        if self.0.data.len() == 1 {
            if self.1.is_empty() {
                unsafe {
                    Complex::from(*self.0.data.get_unchecked(0) as RF)
                }
            } else {
                panic!("cannot discard units")
            }
    ...
    fn to_cb(self: Box<Self>) -> CB {
        if self.0.data.len() == 1 {
            if self.1.is_empty() {
                unsafe {
                    Complex::from(BigInt::from(*self.0.data.get_unchecked(0)))
                }
            } else {
                panic!("cannot discard units")
            }
    ...
    fn to_cfm(self: Box<Self>) -> CFM {
        if self.1.is_empty() {
            self.0.map(|x| Complex::from(x as RF))
        } else {
            panic!("cannot discard units")
        }
    }
    ...
    fn to_ucf(self: Box<Self>) -> UCF {
        if self.0.data.len() == 1 {
            unsafe {
                UT(Complex::from(*self.0.data.get_unchecked(0) as RF),self.1)
            }
        } else if self.0.data.len() == 0 {
            panic!("cannot convert empty matrix")
        } else {
    ...
    fn to_ucb(self: Box<Self>) -> UCB {
        if self.0.data.len() == 1 {
            unsafe {
                UT(Complex::from(BigInt::from(*self.0.data.get_unchecked(0))),self.1)
            }
        } else if self.0.data.len() == 0 {
            panic!("cannot convert empty matrix")
        } else {
    ...
    fn to_ucfm(self: Box<Self>) -> UCFM {
        UT(self.0.map(|x| Complex::from(x as RF)),self.1)
    }
