...
impl Numeric for URF {
    ...
    fn to_rim(self: Box<Self>) -> RIM {
        if self.1.is_empty() {
            DMatrix::from_element(1,1,self.0 as RI)
        } else {
            panic!("cannot discard units")
        }
    }
    ...
    fn to_uri(self: Box<Self>) -> URI {
        UT(self.0 as RI,self.1)
    }
    fn to_urf(self: Box<Self>) -> URF {
        *self
    }
    fn to_urb(self: Box<Self>) -> URB {
        UT(BigInt::from(self.0 as RI),self.1)
    }
    fn to_urim(self: Box<Self>) -> URIM {
        UT(DMatrix::from_element(1,1,self.0 as RI),self.1)
    }
    ...
    fn to_cb(self: Box<Self>) -> CB {
        if self.1.is_empty() {
            Complex::from(BigInt::from(self.0 as RI))
        } else {
            panic!("cannot discard units")
        }
    }
    fn to_cim(self: Box<Self>) -> CIM {
        if self.1.is_empty() {
            DMatrix::from_element(1,1,Complex::from(self.0 as RI))
        } else {
            panic!("cannot discard units")
        }
    }
    fn to_cfm(self: Box<Self>) -> CFM {
        if self.1.is_empty() {
            DMatrix::from_element(1,1,Complex::from(self.0))
        } else {
            panic!("cannot discard units")
        }
    }
    fn to_uci(self: Box<Self>) -> UCI {
        UT(Complex::from(self.0 as RI),self.1)
    }
    fn to_ucf(self: Box<Self>) -> UCF {
        UT(Complex::from(self.0),self.1)
    }
    fn to_ucb(self: Box<Self>) -> UCB {
        UT(Complex::from(BigInt::from(self.0 as RI)),self.1)
    }
    fn to_ucim(self: Box<Self>) -> UCIM {
        UT(DMatrix::from_element(1,1,Complex::from(self.0 as RI)),self.1)
    }
    fn to_ucfm(self: Box<Self>) -> UCFM {
        UT(DMatrix::from_element(1,1,Complex::from(self.0)),self.1)
    }
