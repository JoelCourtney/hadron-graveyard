...
impl Numeric for RFM {
    ...
    fn to_ri(self: Box<Self>) -> RI {
        if self.data.len() == 1 {
            unsafe {
                *self.data.get_unchecked(0) as RI
            }
    ...
    fn to_rb(self: Box<Self>) -> RB {
        if self.data.len() == 1 {
            unsafe {
                BigInt::from(*self.data.get_unchecked(0) as RI)
            }
        } else if self.data.len() == 0 {
            panic!("cannot convert empty matrix")
        } else {
    ...
    fn to_uri(self: Box<Self>) -> URI {
        if self.data.len() == 1 {
            unsafe {
                UT(*self.data.get_unchecked(0) as RI,D::empty())
            }
        } else if self.data.len() == 0 {
            panic!("cannot convert empty matrix")
        } else {
    ...
    fn to_urb(self: Box<Self>) -> URB {
        if self.data.len() == 1 {
            unsafe {
                UT(BigInt::from(*self.data.get_unchecked(0) as RI),D::empty())
            }
        } else if self.data.len() == 0 {
            panic!("cannot convert empty matrix")
        } else {
    ...
    fn to_ci(self: Box<Self>) -> CI {
        if self.data.len() == 1 {
            unsafe {
                Complex::from(*self.data.get_unchecked(0) as RI)
            }
        } else if self.data.len() == 0 {
            panic!("cannot convert empty matrix")
        } else {
    ...
    fn to_cb(self: Box<Self>) -> CB {
        if self.data.len() == 1 {
            unsafe {
                Complex::from(BigInt::from(*self.data.get_unchecked(0) as RI))
            }
        } else if self.data.len() == 0 {
            panic!("cannot convert empty matrix")
        } else {
    ...
    fn to_cim(self: Box<Self>) -> CIM {
        self.map(|x| Complex::from(x as RI))
    }
    ...
    fn to_uci(self: Box<Self>) -> UCI {
        if self.data.len() == 1 {
            unsafe {
                UT(Complex::from(*self.data.get_unchecked(0) as RI),D::empty())
            }
        } else if self.data.len() == 0 {
            panic!("cannot convert empty matrix")
        } else {
    ...
    fn to_ucb(self: Box<Self>) -> UCB {
        if self.data.len() == 1 {
            unsafe {
                UT(Complex::from(BigInt::from(*self.data.get_unchecked(0) as RI)),D::empty())
            }
        } else if self.data.len() == 0 {
            panic!("cannot convert empty matrix")
        } else {
    ...
    fn to_ucim(self: Box<Self>) -> UCIM {
        UT(self.map(|x| Complex::from(x as RI)),D::empty())
    }
