 ...
 * SOFTWARE.
 */
package net.joelcourtney.hadron.launcher;
...
public final class HadronMain {

    private static final String hadron = "hadron";
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (file == null) {
            // @formatter:off
            source = Source.newBuilder(hadron, new InputStreamReader(System.in), "<stdin>").build();
            // @formatter:on
        } else {
            source = Source.newBuilder(hadron, new File(file)).build();
        }
    ...
    private static int executeSource(Source source, InputStream in, PrintStream out, Map<String, String> options) {
        ...
        try {
            context = Context.newBuilder(hadron).in(in).out(out).options(options).build();
        ...
        try {
            ...
            if (context.getBindings(hadron).getMember("main") == null) {
