...
import net.joelcourtney.hadron.test.HadronTestRunner.TestCase;

public class HadronTestRunner extends ParentRunner<TestCase> {

    private static final String SOURCE_SUFFIX = ".hn";
    ...
    public HadronTestRunner(Class<?> runningClass) throws InitializationError {
        super(runningClass);
        try {
    ...
    protected static List<TestCase> createTests(final Class<?> c) throws IOException, InitializationError {
        HadronTestSuite suite = c.getAnnotation(HadronTestSuite.class);
        if (suite == null) {
            throw new InitializationError(String.format("@%s annotation required on class '%s' to run with '%s'.", HadronTestSuite.class.getSimpleName(), c.getName(), HadronTestRunner.class.getSimpleName()));
        }

        String[] paths = suite.value();

        Class<?> testCaseDirectory = c;
        if (suite.testCaseDirectory() != HadronTestSuite.class) {
    ...
    private static final List<NodeFactory<? extends HadronBuiltinNode>> builtins = new ArrayList<>();
    ...
    protected void runChild(TestCase testCase, RunNotifier notifier) {
        ...
        try {
            ...
            for (NodeFactory<? extends HadronBuiltinNode> builtin : builtins) {
                HadronLanguage.installBuiltin(builtin);
            }
    ...
    private static void run(Context context, Path path, PrintWriter out) throws IOException {
        try {
            ...
            Source source = Source.newBuilder(HadronLanguage.ID, path.toFile()).interactive(true).build();
    ...
    public static void runInMain(Class<?> testClass, String[] args) throws InitializationError, NoTestsRemainException {
        ...
        HadronTestRunner suite = new HadronTestRunner(testClass);
        if (args.length > 0) {
