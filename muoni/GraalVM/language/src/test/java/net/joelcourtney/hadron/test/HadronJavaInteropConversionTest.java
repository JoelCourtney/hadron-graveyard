...
public class HadronJavaInteropConversionTest {
    ...
    public void testGR7318Object() throws Exception {
        String sourceText = "fn test(validator) {\n" +
                        "  obj = new();\n" +
                        "  obj.a = new();\n" +
                        "  obj.b = new();\n" +
                        "  return validator.validateObject(obj, obj);\n" +
                        "}";
        try (Context context = Context.newBuilder(HadronLanguage.ID).build()) {
            context.eval(Source.newBuilder(HadronLanguage.ID, sourceText, "Test").build());
            Value test = context.getBindings(HadronLanguage.ID).getMember("test");
    ...
    public void testGR7318List() throws Exception {
        String sourceText = "fn test(validator, array) {\n" +
                        "  array[0] = new();\n" +
                        "  array[1] = new();\n" +
                        "  return validator.validateList(array, array);\n" +
                        "}";
        try (Context context = Context.newBuilder(HadronLanguage.ID).allowHostAccess(HostAccess.ALL).build()) {
            context.eval(Source.newBuilder(HadronLanguage.ID, sourceText, "Test").build());
            Value test = context.getBindings(HadronLanguage.ID).getMember("test");
