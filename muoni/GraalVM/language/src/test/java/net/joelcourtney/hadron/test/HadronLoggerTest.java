...
public class HadronLoggerTest {

    private static final Source ADD_SL;
    private static final Source MUL_SL;
    static {
        ADD_SL = Source.newBuilder("hadron", "fn add(a,b) {return a + b;} fn main() {return add(1,1);}", "add.sl").buildLiteral();
        MUL_SL = Source.newBuilder("hadron", "fn mul(a,b) {return a * b;} fn main() {return mul(1,1);}", "mul.sl").buildLiteral();
    }
    ...
    private Context createContext(Map<String, String> options) {
        ...
        currentContext = Context.newBuilder("hadron").options(options).logHandler(testHandler).build();
    ...
    public void testLoggerSlFunctionLevelFine() {
        final Context context = createContext(createLoggingOptions("hadron", "net.joelcourtney.hadron.runtime.HadronFunction", "FINE"));
        executeSlScript(context);
    ...
    public void testLoggerSlFunctionParentLevelFine() {
        final Context context = createContext(createLoggingOptions("hadron", "net.joelcourtney.hadron.runtime", "FINE"));
        executeSlScript(context);
        Assert.assertFalse(functionNames(testHandler.getRecords()).isEmpty());
    }
    ...
    public void testLoggerSlFunctionSiblingLevelFine() {
        final Context context = createContext(createLoggingOptions("hadron", "net.joelcourtney.hadron.runtime.HadronContext", "FINE"));
        executeSlScript(context);
        Assert.assertTrue(functionNames(testHandler.getRecords()).isEmpty());
    }
    ...
    public void testMultipleContextsExclusiveFineLevel() {
        ...
        try (Context ctx = Context.newBuilder("hadron").options(createLoggingOptions("hadron", "net.joelcourtney.hadron.runtime.HadronFunction", "FINE")).logHandler(handler1).build()) {
            executeSlScript(ctx, ADD_SL, 2);
        }
        final TestHandler handler2 = new TestHandler();
        try (Context ctx = Context.newBuilder("hadron").options(createLoggingOptions("hadron", "net.joelcourtney.hadron.runtime.HadronFunction", "FINE")).logHandler(handler2).build()) {
            executeSlScript(ctx, MUL_SL, 1);
        }
        final TestHandler handler3 = new TestHandler();
        try (Context ctx = Context.newBuilder("hadron").options(createLoggingOptions("hadron", "net.joelcourtney.hadron.runtime.HadronFunction", "FINE")).logHandler(handler3).build()) {
            executeSlScript(ctx, ADD_SL, 2);
        }
    ...
    public void testMultipleContextsExclusiveDifferentLogLevel() {
        ...
        final TestHandler handler2 = new TestHandler();
        try (Context ctx = Context.newBuilder("hadron").logHandler(handler2).build()) {
            executeSlScript(ctx, MUL_SL, 1);
        }
        final TestHandler handler3 = new TestHandler();
        try (Context ctx = Context.newBuilder("hadron").options(createLoggingOptions("hadron", "net.joelcourtney.hadron.runtime.HadronFunction", "FINE")).logHandler(handler3).build()) {
    ...
    public void testMultipleContextsNestedFineLevel() {
        ...
        final TestHandler handler2 = new TestHandler();
        final TestHandler handler3 = new TestHandler();
        try (Context ctx1 = Context.newBuilder("hadron").options(createLoggingOptions("hadron", "net.joelcourtney.hadron.runtime.HadronFunction", "FINE")).logHandler(handler1).build()) {
            try (Context ctx2 = Context.newBuilder("hadron").options(createLoggingOptions("hadron", "net.joelcourtney.hadron.runtime.HadronFunction", "FINE")).logHandler(handler2).build()) {
                try (Context ctx3 = Context.newBuilder("hadron").options(createLoggingOptions("hadron", "net.joelcourtney.hadron.runtime.HadronFunction", "FINE")).logHandler(handler3).build()) {
    ...
    public void testMultipleContextsNestedDifferentLogLevel() {
        ...
        try (Context ctx1 = Context.newBuilder("hadron").options(createLoggingOptions("hadron", "net.joelcourtney.hadron.runtime.HadronFunction", "FINE")).logHandler(handler1).build()) {
            try (Context ctx2 = Context.newBuilder("hadron").options(createLoggingOptions("hadron", "net.joelcourtney.hadron.runtime.HadronFunction", "FINE")).logHandler(handler2).build()) {
                try (Context ctx3 = Context.newBuilder("hadron").logHandler(handler3).build()) {
                    executeSlScript(ctx1, ADD_SL, 2);
                    executeSlScript(ctx2, MUL_SL, 1);
                    executeSlScript(ctx3, ADD_SL, 2);
                }
    ...
    private static Set<String> functionNames(final List<? extends LogRecord> records) {
        return records.stream().filter((lr) -> "hadron.net.joelcourtney.hadron.runtime.HadronFunction".equals(lr.getLoggerName())).map((lr) -> (String) lr.getParameters()[0]).collect(Collectors.toSet());
    }
