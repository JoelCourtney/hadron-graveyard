...
public class HadronSharedCodeSeparatedEnvTest {
    ...
    public void initializeEngines() {
        ...
        int instances = HadronLanguage.counter;
        // @formatter:off
        e1 = Context.newBuilder("hadron").engine(engine).out(os1).allowPolyglotAccess(PolyglotAccess.ALL).build();
        e1.getPolyglotBindings().putMember("extra", 1);
        e2 = Context.newBuilder("hadron").engine(engine).out(os2).allowPolyglotAccess(PolyglotAccess.ALL).build();
        e2.getPolyglotBindings().putMember("extra", 2);
        e1.initialize("hadron");
        e2.initialize("hadron");
        assertEquals("One HadronLanguage instance created", instances + 1, HadronLanguage.counter);
    }
    ...
    public void shareCodeUseDifferentOutputStreams() throws Exception {
        ...
        e1.eval("hadron", sayHello);
        assertEquals("Ahoj1\n", toUnixString(os1));
        assertEquals("", toUnixString(os2));

        e2.eval("hadron", sayHello);
        assertEquals("Ahoj1\n", toUnixString(os1));
        assertEquals("Ahoj2\n", toUnixString(os2));
    }
