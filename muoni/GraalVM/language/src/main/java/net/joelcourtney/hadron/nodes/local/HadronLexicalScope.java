 ...
 * SOFTWARE.
 */
package net.joelcourtney.hadron.nodes.local;
...
import net.joelcourtney.hadron.nodes.controlflow.HadronBlockNode;
import net.joelcourtney.hadron.runtime.HadronNull;

/**
 * Simple language lexical scope. There can be a block scope, or function scope.
 */
public final class HadronLexicalScope {

    private final Node current;
    private final HadronBlockNode block;
    private final HadronBlockNode parentBlock;
    private final RootNode root;
    private HadronLexicalScope parent;
     ...
     */
    private HadronLexicalScope(Node current, HadronBlockNode block, HadronBlockNode parentBlock) {
     ...
     */
    private HadronLexicalScope(Node current, HadronBlockNode block, RootNode root) {
        this.current = current;
        this.block = block;
    ...
    public static HadronLexicalScope createScope(Node node) {
        HadronBlockNode block = getParentBlock(node);
        if (block == null) {
            // We're in the root.
            block = findChildrenBlock(node);
            if (block == null) {
                // Corrupted SL AST, no block was found
                assert node.getRootNode() instanceof HadronEvalRootNode : "Corrupted SL AST under " + node;
                return new HadronLexicalScope(null, null, (HadronBlockNode) null);
            }
            node = null; // node is above the block
        }
        // Test if there is a parent block. If not, we're in the root scope.
        HadronBlockNode parentBlock = getParentBlock(block);
        if (parentBlock == null) {
            return new HadronLexicalScope(node, block, block.getRootNode());
        } else {
            return new HadronLexicalScope(node, block, parentBlock);
        }
    }

    private static HadronBlockNode getParentBlock(Node node) {
        HadronBlockNode block;
        Node parent = node.getParent();
        // Find a nearest block node.
        while (parent != null && !(parent instanceof HadronBlockNode)) {
            parent = parent.getParent();
        }
        if (parent != null) {
            block = (HadronBlockNode) parent;
        } else {
            block = null;
        }
        return block;
    ...
    private static HadronBlockNode findChildrenBlock(Node node) {
        HadronBlockNode[] blockPtr = new HadronBlockNode[1];
        node.accept(new NodeVisitor() {
            @Override
            public boolean visit(Node n) {
                if (n instanceof HadronBlockNode) {
                    blockPtr[0] = (HadronBlockNode) n;
                    return false;
                } else {
                    return true;
                }
    ...
    public HadronLexicalScope findParent() {
        if (parentBlock == null) {
        ...
        if (parent == null) {
            Node node = block;
            HadronBlockNode newBlock = parentBlock;
            // Test if there is a next parent block. If not, we're in the root scope.
            HadronBlockNode newParentBlock = getParentBlock(newBlock);
            if (newParentBlock == null) {
                parent = new HadronLexicalScope(node, newBlock, newBlock.getRootNode());
            } else {
                parent = new HadronLexicalScope(node, newBlock, newParentBlock);
            }
        }
        return parent;
    }
    ...
    private Map<String, FrameSlot> collectVars(Node varsBlock, Node currentNode) {
        ...
        // with HadronWriteLocalVariableNode. The traversal stops when we hit the current node.
        ...
        NodeUtil.forEachChild(varsBlock, new NodeVisitor() {
            ...
            public boolean visit(Node node) {
                ...
                if (!(node instanceof HadronBlockNode)) {
                ...
                if (node instanceof HadronWriteLocalVariableNode) {
                    HadronWriteLocalVariableNode wn = (HadronWriteLocalVariableNode) node;
    ...
    private static Map<String, FrameSlot> collectArgs(Node block) {
        ...
        // To collect argument slots, search for HadronReadArgumentNode inside of
        // HadronWriteLocalVariableNode.
        Map<String, FrameSlot> args = new LinkedHashMap<>(4);
        NodeUtil.forEachChild(block, new NodeVisitor() {

            private HadronWriteLocalVariableNode wn; // The current write node containing a slot

            @Override
            public boolean visit(Node node) {
                // When there is a write node, search for HadronReadArgumentNode among its children:
                if (node instanceof HadronWriteLocalVariableNode) {
                    wn = (HadronWriteLocalVariableNode) node;
                    boolean all = NodeUtil.forEachChild(node, this);
                    wn = null;
                    return all;
                } else if (wn != null && (node instanceof HadronReadArgumentNode)) {
                    ...
                    args.put(name, slot);
                    return true;
                } else if (wn == null && (node instanceof HadronStatementNode)) {
