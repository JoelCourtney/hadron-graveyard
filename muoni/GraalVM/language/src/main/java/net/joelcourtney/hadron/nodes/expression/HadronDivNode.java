...
/**
 * This class is similar to the extensively documented {@link HadronAddNode}. Divisions by 0 throw the
 * same {@link ArithmeticException exception} as in Java, SL has no special handling for it to keep
 * the code simple.
 */
@NodeInfo(shortName = "/")
public abstract class HadronDivNode extends HadronBinaryNode {
    ...
    @Specialization
    @TruffleBoundary
    protected HadronBigNumber div(HadronBigNumber left, HadronBigNumber right) {
        return new HadronBigNumber(left.getValue().divide(right.getValue()));
    }
