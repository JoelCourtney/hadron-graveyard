 ...
 * are automatically generated by the Truffle DSL. In addition, a {@link HadronAddNodeGen factory class}
 * is generated that provides, e.g., {@link HadronAddNodeGen#create node creation}.
 */
@NodeInfo(shortName = "+")
public abstract class HadronAddNode extends HadronBinaryNode {
     ...
     * the {@link #add(HadronBigNumber, HadronBigNumber) slow path}. Therefore, we use an
    ...
    /**
     * This is the slow path of the arbitrary-precision arithmetic. The {@link HadronBigNumber} type of
     * Java is doing everything we need.
     * <p>
     * This specialization is automatically selected by the Truffle DSL if both the left and right
     * operand are {@link HadronBigNumber} values. Because the type system defines an
     * {@link ImplicitCast implicit conversion} from {@code long} to {@link HadronBigNumber} in
     * {@link HadronTypes#castBigNumber(long)}, this specialization is also taken if the left or the
    ...
    @Specialization
    @TruffleBoundary
    protected HadronBigNumber add(HadronBigNumber left, HadronBigNumber right) {
        return new HadronBigNumber(left.getValue().add(right.getValue()));
    }
