 ...
 * {@link HadronLogicalNotNode negate} the {@code ==} operator.
 */
@NodeInfo(shortName = "==")
public abstract class HadronEqualNode extends HadronBinaryNode {
    ...
    @Specialization
    @TruffleBoundary
    protected boolean equal(HadronBigNumber left, HadronBigNumber right) {
    ...
    @Specialization
    protected boolean equal(HadronFunction left, HadronFunction right) {
        /*
         * Our function registry maintains one canonical HadronFunction object per function name, so we
         * do not need equals().
         */
        return left == right;
    }
    ...
    @Specialization
    protected boolean equal(HadronNull left, HadronNull right) {
        /* There is only the singleton instance of HadronNull, so we do not need equals(). */
        return left == right;
    }
