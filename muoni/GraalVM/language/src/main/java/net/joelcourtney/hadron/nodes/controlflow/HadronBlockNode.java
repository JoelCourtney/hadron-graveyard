...
public final class HadronBlockNode extends HadronExpressionNode {
     ...
     */
    @Children private final HadronStatementNode[] bodyNodes;

    public HadronBlockNode(HadronStatementNode[] bodyNodes) {
        this.bodyNodes = bodyNodes;
    }

    /**
     * Execute all child statements. The annotation {@link ExplodeLoop} triggers full unrolling of
     * the loop during compilation. This allows the {@link HadronStatementNode#executeVoid} method of
    ...
    public Object executeGeneric(VirtualFrame frame) {
        ...
        if (bodyNodes.length > 0) {
            HadronStatementNode lastNode = bodyNodes[bodyNodes.length - 1];
            if (lastNode instanceof HadronExpressionNode) {
                return ((HadronExpressionNode) lastNode).executeGeneric(frame);
            } else {
                lastNode.executeVoid(frame);
            }
        }
        return HadronNull.SINGLETON;
    }

    public List<HadronStatementNode> getStatements() {
